...
function VisualizacionPoste({ radio, datosCable, setDatosCable, onCalcularNudo, tasaCambio }: Props) {
  const MIN_SIZE = 1;
    ...
    <div className="h-full min-h-[400px] flex flex-col items-center justify-center bg-gray-50 rounded-lg p-4">
      ...
      <div className="space-y-2 text-center mt-4">
        ...
        <p className="text-sm font-medium text-blue-600">
          USD/CLP: ${tasaCambio.toLocaleString('es-CL')}
          <span className="text-xs ml-1">(Currency API)</span>
        </p>
      </div>
    </div>
  );
...
export default VisualizacionPoste;